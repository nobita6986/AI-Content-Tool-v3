...
export const LoadingOverlay: React.FC<{ message?: string }> = ({ message = "Đang tạo..." }) => (
    <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center rounded-xl z-10 transition-all duration-300">
        <div className="flex flex-col items-center gap-3 bg-slate-950/80 p-4 rounded-xl border border-sky-900/50 shadow-2xl">
...
export const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode; maxWidth?: string }> = ({ isOpen, onClose, title, children, maxWidth = "max-w-lg" }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className={`w-full ${maxWidth} rounded-2xl bg-slate-900 border border-sky-900 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200`}>
